		...
		try {
			...
			if (settings.isStrict()) {
				...
				if (sessionExpiration != null) {
					sessionExpiration = sessionExpiration.plus(settings.getAllowedClockDrift() * 1000);
		...
		for (int i = 0; i < subjectConfirmationNodes.getLength(); i++) {
			...
			for (int c = 0; c < subjectConfirmationDataNodes.getLength(); c++) {
				if (subjectConfirmationDataNodes.item(c).getLocalName() != null && subjectConfirmationDataNodes.item(c).getLocalName().equals("SubjectConfirmationData")) {
					...
					noa = noa.plus(settings.getAllowedClockDrift() * 1000);
					...
					if (notBefore != null) {
						...
						nb = nb.minus(settings.getAllowedClockDrift() * 1000);
						if (nb.isAfterNow()) {
		...
		if (timestampNodes.getLength() != 0) {
			for (int i = 0; i < timestampNodes.getLength(); i++) {
				...
				if (naAttribute != null) {
					...
					notOnOrAfterDate = notOnOrAfterDate.plus(settings.getAllowedClockDrift() * 1000);
				...
				if (nbAttribute != null) {
					...
					notBeforeDate = notBeforeDate.minus(settings.getAllowedClockDrift() * 1000);
